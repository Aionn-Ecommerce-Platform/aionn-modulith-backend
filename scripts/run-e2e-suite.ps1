param(
    [ValidateSet("all", "identity", "catalog", "inventory", "ordering", "payment")]
    [string]$Module = "all"
)
...
# 2. Load env files
Get-Content envs/common.env, envs/identity.env, envs/catalog.env, envs/inventory.env, envs/ordering.env, envs/payment.env | ForEach-Object {
    $line = $_.Trim()

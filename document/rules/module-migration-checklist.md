...
- [ ] Use the next free major version. Currently taken: `V1.x` identity, `V2.x` catalog, `V3.x` inventory, `V4.x` ordering, `V5.x` payment, `V6.x` shipping, `V8.x` promotion, `V9.x` notification. `V7.x` is permanently unused: it was reserved for notification before promotion landed on `V8.0`, and back-filling it now would be an out-of-order migration that fails startup on any database already past `V8.0`. Always take the next version **above** the highest applied one.

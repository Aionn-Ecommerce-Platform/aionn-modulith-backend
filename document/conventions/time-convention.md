...
- **Retrieving Current Time**: Always use `clock.instant()`. Never call `Instant.now()`, and never call `Instant.now(clock)` either — the ArchUnit rule `NO_BARE_INSTANT_NOW_IN_DOMAIN` (asserted by `SharedKernelArchitectureTest`) rejects **both** forms in domain classes and fails the build. `Instant.now(clock)` behaves identically to `clock.instant()`, so there is never a reason to prefer it.
- **Folding Vietnamese Text**: NFD normalisation plus combining-mark stripping does **not** decompose `đ`/`Đ` (U+0111 / U+0110); they are single code points, not base-plus-diacritic compositions. Any place-name matching must fold them explicitly with `.replace('\u0111', 'd')` after lowercasing, otherwise `Ba Đình`, `Đà Nẵng` and `Đồng Nai` never match.

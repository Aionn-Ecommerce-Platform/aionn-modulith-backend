...
public class VerifyEmailUseCase implements VerifyEmailInputPort {
    ...
    @Override
    @Transactional
    public void requestOtp(RequestEmailVerificationOtpCommand command) {
        accountManagementService.sendVerifyPrimaryEmailOtp(command.userId());
    }

    @Override
    @Transactional
    public void confirm(ConfirmEmailVerificationCommand command) {
        accountManagementService.confirmVerifyPrimaryEmailOtp(command.userId(), command.otpCode());
    }
}
